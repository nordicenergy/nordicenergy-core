...
case "${ngy_PATH+set}" in
"")
   unset -v gopath
   gopath=$(go env GOPATH)
   # ngy_PATH is the common root directory of all nordicenergy repos
   ngy_PATH="${gopath%%:*}/src/github.com/nordicenergy"
   if [ ! -d $ngy_PATH ]; then
      # "env pwd" uses external pwd(1) implementation and not the Bash built-in,
      # which does not fully dereference symlinks.
      ngy_PATH=$(cd $progdir/../.. && env pwd)
   fi
   ;;
esac
: ${OPENSSL_DIR="/usr/local/opt/openssl"}
: ${MCL_DIR="${ngy_PATH}/mcl"}
: ${BLS_DIR="${ngy_PATH}/bls"}
