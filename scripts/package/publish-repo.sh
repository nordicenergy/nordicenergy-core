...
declare -A TARGET
TARGET[rpm.dev]="haochen-nordicenergy-pub/pub/yum"
TARGET[deb.dev]="haochen-nordicenergy-pub/pub/repo"
TARGET[rpm.prod]="pub.nordicenergy.net/release/package/yum"
TARGET[deb.prod]="pub.nordicenergy.net/release/package/apt"
...
function publish_deb() {
   if aptly repo show nordicenergy-$PROFILE > /dev/null; then
      aptly repo add nordicenergy-$PROFILE $SRC
      aptly publish update bionic s3:nordicenergy-$PROFILE:
   else
      aptly repo create -distribution=bionic -compnetnt=main nordicenergy-$PROFILE
      aptly repo add nordicenergy-$PROFILE $SRC
      aptly publish repo nordicenergy-$PROFILE s3:nordicenergy-$PROFILE:
   fi
}
