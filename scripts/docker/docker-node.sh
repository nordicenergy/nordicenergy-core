...
DOCKER_REPO=nordicenergynet/node
...
options:
  ...
  -z dns_znet : dns znet
  -d db_dir   : nordicenergy db directory
...
if [ -n "$(docker ps -q -a -f name=^nordicenergy-${tag}-${port_base}$)" ]; then
  echo "Stop node for tag: $tag, port: $port_base"
  docker rm -v -f nordicenergy-${tag}-${port_base} >/dev/null
...
mkdir -p ${db_dir}/nordicenergy_db_0
mkdir -p ${db_dir}/nordicenergy_db_1
mkdir -p ${db_dir}/nordicenergy_db_2
mkdir -p ${db_dir}/nordicenergy_db_3

docker run -it -d \
  --name nordicenergy-$tag-$port_base \
  ...
  -v $(realpath ${db_dir}/nordicenergy_db_0):/nordicenergy/nordicenergy_db_0 \
  -v $(realpath ${db_dir}/nordicenergy_db_1):/nordicenergy/nordicenergy_db_1 \
  -v $(realpath ${db_dir}/nordicenergy_db_2):/nordicenergy/nordicenergy_db_2 \
  -v $(realpath ${db_dir}/nordicenergy_db_3):/nordicenergy/nordicenergy_db_3 \
  -v $(realpath keys):/nordicenergy/.ngy \
  -v $(realpath logs):/nordicenergy/log \
  $DOCKER_IMAGE >/dev/null

echo
echo "======================================"
echo "Node for tag ($tag) (port $port_base) is running in container 'nordicenergy-$tag-$port_base'"
echo
echo "To check console log, please run \`docker logs -f nordicenergy-$tag-$port_base\`"
