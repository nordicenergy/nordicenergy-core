# Docker deployment of a Rosetta enabled NordicEnergy node

## Docker Image
You can choose to build the docker image using the included Dockerfile with the following command:
```bash
docker build -t nordicenergy/explorer-node . 
```

Or you can download/pull the image from dockerhub with the following command:
```bash
docker pull nordicenergy/explorer-node:latest
```

## Starting the node
You can start the node with the following command:
```bash
docker run -d -p 9700:9700 -v "$(pwd)/data:/root/data" nordicenergy/explorer-node --run.shard=0 
```
> This command will create the container of the NordicEnergy node on shard 0 in the detached mode, 
...
> uses `/root/data` for all data storage (this is where the `nordicenergy_db_*` directories will be stored).
...
```bash
docker run -d -p 9700:9700 -v "$(pwd)/data:/root/data" nordicenergy/explorer-node --run.shard=0 --run.offline 
```
...
**Note that all the arguments provided when running the docker img are immediately forwarded to the NordicEnergy node binary.**
...
All the args on the image run are forwarded to the NordicEnergy node binary. Therefore, you can simply add `-n testnet` to 
run the node for testnet. For example:
```bash 
docker run -d -p 9700:9700 -v "$(pwd)/data:/root/data" nordicenergy/explorer-node --run.shard=0 -n testnet
```
...
```bash
docker run -d -p 9700:9700 -p 9500:9500 -v "$(pwd)/data:/root/data" nordicenergy/explorer-node --run.shard=0 -n testnet --http
```
...
the host port to the container's rpc server port.
```bash
docker run -d -p 9700:9700 -p 9800:9900 -v "$(pwd)/data:/root/data" nordicenergy/explorer-node --run.shard=0 -n testnet --ws
```
...
```bash 
docker run -d -p 9700:9700 -v "$(pwd)/data:/root/data" nordicenergy/explorer-node --run.shard=0 -n testnet --run.archive=false
```
...
the `NordicEnergy_db_*` directory to some directory (i.e: `./data`) and mount the volume on the docker run. 
...
├── logs
│    ├── node_execution.log
│    └── zerolog-NordicEnergy.log
└── transactions.rlp
``` 

### Inspecting Logs
If you mount `./data` on the host to `/root/data` in the container, you van view the NordicEnergy node logs at
