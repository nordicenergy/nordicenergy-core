...
// Note that this test only checks the general format of each type transaction on nordicenergy.
// The detailed operation checks for each type of transaction is dnet in separate unit tests.
...
func testFormatStakingTransaction(
	...
	receipt := &ngytypes.Receipt{
		Status:  ngytypes.ReceiptStatusSuccessful,
		GasUsed: gasUsed,
	}
...
func testFormatPlainTransaction(
	...
	signer := ngytypes.HomesteadSigner{}
