...
// GetNativeOperationsFromTransaction for net of the following transactions:
...
func GetNativeOperationsFromTransaction(
	tx *ngytypes.Transaction, receipt *ngytypes.Receipt, contractInfo *ContractInfo,
...
func GetNativeOperationsFromStakingTransaction(
	tx *stakingTypes.StakingTransaction, receipt *ngytypes.Receipt,
) ([]*types.Operation, *types.Error) {
	senderAddress, err := tx.SenderAddress()
	if err != nil {
		senderAddress = FormatDefaultSenderAddress
...
func GetSideEffectOperationsFromUndelegationPayouts(
	payouts ngy.UndelegationPayouts, startingOperationIndex *int64,
) ([]*types.Operation, *types.Error) {
...
func GetSideEffectOperationsFromPreStakingRewards(
	rewards ngy.PreStakingBlockRewards, startingOperationIndex *int64,
) ([]*types.Operation, *types.Error) {
	return getSideEffectOperationsFromValueMap(
...
// GetTransactionStatus for any valid nordicenergy transaction given its receipt.
func GetTransactionStatus(tx ngytypes.PoolTransaction, receipt *ngytypes.Receipt) string {
	if _, ok := tx.(*ngytypes.Transaction); ok {
		status := common.SuccessOperationStatus.Status
		if receipt.Status == ngytypes.ReceiptStatusFailed {
...
func getBasicTransferNativeOperations(
	tx *ngytypes.Transaction, receipt *ngytypes.Receipt, senderAddress ethcommon.Address, toAddress *ethcommon.Address,
...
func getContractCreationNativeOperations(
	tx *ngytypes.Transaction, receipt *ngytypes.Receipt, senderAddress ethcommon.Address, contractInfo *ContractInfo,
	startingOperationIndex *int64,
) ([]*types.Operation, *types.Error) {
	basicOps, rosettaError := getBasicTransferNativeOperations(
...
func getContractInternalTransferNativeOperations(
	executionResult *ngy.ExecutionResult, status string,
	startingOperationIndex *int64,
) ([]*types.Operation, *types.Error) {
	ops := []*types.Operation{}
...
func getCrossShardSenderTransferNativeOperations(
	tx *ngytypes.Transaction, senderAddress ethcommon.Address,
	startingOperationIndex *int64,
) ([]*types.Operation, *types.Error) {
	if tx.To() == nil {
...
func getAmountFromDelegateMessage(receipt *ngytypes.Receipt, data []byte) (*types.Amount, *types.Error) {
	...
	logs := ngytypes.FindLogsWithTopic(receipt, staking.DelegateTopic)
...
func getAmountFromCollectRewards(
	receipt *ngytypes.Receipt, senderAddress ethcommon.Address,
) (*types.Amount, *types.Error) {
	var amount *types.Amount
	logs := ngytypes.FindLogsWithTopic(receipt, staking.CollectRewardsTopic)
	for _, log := range logs {
