...
func NewNetworkAPI(ngy *ngy.nordicenergy) server.NetworkAPIServicer {
	return &NetworkAPI{
		ngy: ngy,
	}
}
...
func (s *NetworkAPI) NetworkList(
	...
	network, err := common.GetNetwork(s.ngy.ShardID)
	if err != nil {
...
func (s *NetworkAPI) NetworkStatus(
	...
	currBlock := s.ngy.CurrentBlock()
	var currentHeader *block.Header
	var err error
	if currBlock.Number().Cmp(big.NewInt(0)) == 1 && !s.ngy.IsStakingEpoch(currBlock.Epoch()) {
		...
		currentHeader, err = s.ngy.HeaderByNumber(ctx, rpc.BlockNumber(blkNum.Uint64()))
	} else {
		currentHeader, err = s.ngy.HeaderByNumber(ctx, rpc.LatestBlockNumber)
	}
	if err != nil {
		return nil, common.NewError(common.CatchAllError, map[string]interface{}{
	...
	genesisHeader, err := s.ngy.HeaderByNumber(ctx, rpc.BlockNumber(0))
	if err != nil {
		return nil, common.NewError(common.CatchAllError, map[string]interface{}{
			"message": fmt.Sprintf("unable to get genesis header: %v", err.Error()),
		})
	}
	peers, rosettaError := getPeersFromNodePeerInfo(s.ngy.GetPeerInfo())
	if rosettaError != nil {
		return nil, rosettaError
	}
	targetHeight := int64(s.ngy.NodeAPI.GetMaxPeerHeight())
	syncStatus := common.SyncingFinish
	if s.ngy.NodeAPI.IsOutOfSync(s.ngy.BlockChain) {
	...
	if !nodeconfig.GetShardConfig(s.ngy.ShardID).GetArchival() {
		maxGarbCollectedBlockNum := s.ngy.BlockChain.GetMaxGarbageCollectedBlockNumber()
		if maxGarbCollectedBlockNum == -1 || maxGarbCollectedBlockNum >= currentHeader.Number().Int64() {
			...
			oldestBlockHeader, err := s.ngy.HeaderByNumber(ctx, rpc.BlockNumber(maxGarbCollectedBlockNum+1))
			if err != nil {
				return nil, common.NewError(common.CatchAllError, map[string]interface{}{
...
func (s *NetworkAPI) NetworkOptions(
	...
	isArchival := nodeconfig.GetShardConfig(s.ngy.ShardID).GetArchival()
...
func getBeaconAllow(isArchival bool) *types.Allow {
	return &types.Allow{
		...
		Errors:                  append(getErrors(), getBeacnetrrors()...),
...
func getBeacnetrrors() []*types.Error {
	return []*types.Error{
		&common.StakingTransactionSubmissinetrror,
	}
}
