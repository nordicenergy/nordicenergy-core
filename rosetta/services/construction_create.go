...
func unpackWrappedTransactionFromString(
	str string,
) (*WrappedTransaction, ngyTypes.PoolTransaction, *types.Error) {
	wrappedTransaction := &WrappedTransaction{}
	if err := json.Unmarshal([]byte(str), wrappedTransaction); err != nil {
		return nil, nil, common.NewError(common.InvalidTransactionConstructinetrror, map[string]interface{}{
...
func (s *ConstructAPI) ConstructionPayloads(
	...
	if senderID.Address != compnetnts.From.Address {
		return nil, common.NewError(common.InvalidTransactionConstructinetrror, map[string]interface{}{
			"message": "sender account identifier from operations does not match account identifier from public key",
		})
	}
	if metadata.Transaction.FromShardID != nil && *metadata.Transaction.FromShardID != s.ngy.ShardID {
		return nil, common.NewError(common.InvalidTransactionConstructinetrror, map[string]interface{}{
			"message": fmt.Sprintf("transaction is for shard %v != shard %v",
				*metadata.Transaction.FromShardID, s.ngy.ShardID,
			),
		})
	}

	unsignedTx, rosettaError := ConstructTransaction(compnetnts, metadata, s.ngy.ShardID)
	if rosettaError != nil {
		return nil, rosettaError
	}
	...
	wrappedTxMarshalledBytes, err := json.Marshal(WrappedTransaction{
		...
		IsStaking:    compnetnts.IsStaking(),
	})
	if err != nil {
		return nil, common.NewError(common.CatchAllError, map[string]interface{}{
			"message": err.Error(),
...
func (s *ConstructAPI) getSigningPayload(
	tx ngyTypes.PoolTransaction, senderAccountID *types.AccountIdentifier,
	...
	case *ngyTypes.Transaction:
		payload.Bytes = s.signer.Hash(tx.(*ngyTypes.Transaction)).Bytes()
	default:
		return nil, common.NewError(common.CatchAllError, map[string]interface{}{
...
func (s *ConstructAPI) ConstructionCombine(
	...
	var err error
	var signedTx ngyTypes.PoolTransaction
