...
func (s *BlockAPI) containsSideEffectTransaction(
	ctx context.Context, blk *ngytypes.Block,
) bool {
	if blk == nil {
		return false
	}
	return s.ngy.IsCommitteeSelectionBlock(blk.Header()) || !s.ngy.IsStakingEpoch(blk.Epoch()) || blk.NumberU64() == 0
}
...
// getSideEffectTransaction returns the side effect transaction for a block if said block has net.
// Side effects to reports are: genesis funds, undelegation payouts, permissinetd-phase block rewards.
func (s *BlockAPI) getSideEffectTransaction(
	...
	if blk.NumberU64() == 0 {
		ops, rosettaError := GetSideEffectOperationsFromGenesisSpec(core.GetGenesisSpec(s.ngy.ShardID), startingOpIndex)
		if rosettaError != nil {
			return nil, rosettaError
		}
		updateStartingOpIndex(ops)
	}
	// Handle block rewards for epoch < staking epoch (permissinetd-phase block rewards)
	// Note that block rewards don't start until the second block.
	if !s.ngy.IsStakingEpoch(blk.Epoch()) && blk.NumberU64() > 1 {
		rewards, err := s.ngy.GetPreStakingBlockRewards(ctx, blk)
		if err != nil {
	...
	if s.ngy.IsCommitteeSelectionBlock(blk.Header()) && s.ngy.IsPreStakingEpoch(blk.Epoch()) {
		payouts, err := s.ngy.GetUndelegationPayouts(ctx, blk.Epoch())
		if err != nil {
			return nil, common.NewError(common.CatchAllError, map[string]interface{}{
				"message": err.Error(),
			})
...
func (s *BlockAPI) sideEffectBlockTransaction(
	...
	blk, rosettaError := getBlock(ctx, s.ngy, &types.PartialBlockIdentifier{Index: &request.BlockIdentifier.Index})
	if rosettaError != nil {
		return nil, rosettaError
	}
