...
import (
	...
	"github.com/nordicenergy/nordicenergy-core/rpc"
...
func NewBlockAPI(ngy *ngy.nordicenergy) server.BlockAPIServicer {
...
func (s *BlockAPI) Block(
	...
	var blk *ngytypes.Block
	var currBlockID, prevBlockID *types.BlockIdentifier
	if blk, rosettaError = getBlock(ctx, s.ngy, request.BlockIdentifier); rosettaError != nil {
	...
	if blk.NumberU64() == 0 {
		...
		prevBlock, err := s.ngy.BlockByNumber(ctx, rpc.BlockNumber(blk.Number().Int64()-1).EthBlockNumber())
		if err != nil {
...
func (s *BlockAPI) BlockTransaction(
	...
	blk, rosettaError := getBlock(
		ctx, s.ngy, &types.PartialBlockIdentifier{Hash: &request.BlockIdentifier.Hash},
	)
	if rosettaError != nil {
		return nil, rosettaError
	}
	...
	state, _, err := s.ngy.StateAndHeaderByNumber(ctx, rpc.BlockNumber(request.BlockIdentifier.Index).EthBlockNumber())
	...
	if txInfo.tx != nil && txInfo.receipt != nil {
		...
		if _, ok := txInfo.tx.(*ngytypes.Transaction); ok {
...
// transactionInfo stores all related information for any transaction on the nordicenergy chain
// Note that some elements can be nil if not applicable
type transactionInfo struct {
	tx        ngytypes.PoolTransaction
	txIndex   uint64
	receipt   *ngytypes.Receipt
...
func (s *BlockAPI) getTransactionInfo(
	ctx context.Context, blk *ngytypes.Block, txHash ethcommon.Hash,
) (txInfo *transactionInfo, rosettaError *types.Error) {
	// Look for all of the possible transactions
	var index uint64
	var plainTx *ngytypes.Transaction
	var stakingTx *stakingTypes.StakingTransaction
	plainTx, _, _, index = rawdb.ReadTransaction(s.ngy.ChainDb(), txHash)
	if plainTx == nil {
		stakingTx, _, _, index = rawdb.ReadStakingTransaction(s.ngy.ChainDb(), txHash)
		// if there both normal and staking transactions, correct index offset.
		index = index + uint64(blk.Transactions().Len())
	}
	cxReceipt, _, _, _ := rawdb.ReadCXReceipt(s.ngy.ChainDb(), txHash)

	if plainTx == nil && stakingTx == nil && cxReceipt == nil {
		return nil, &common.TransactionNotFoundError
	}

	var receipt *ngytypes.Receipt
	receipts, err := s.ngy.GetReceipts(ctx, blk.Hash())
	if err != nil {
		return nil, common.NewError(common.CatchAllError, map[string]interface{}{
			"message": err.Error(),
		})
...
func (s *BlockAPI) getTransactionTrace(
	ctx context.Context, blk *ngytypes.Block, txInfo *transactionInfo,
) (*ngy.ExecutionResult, *types.Error) {
	cacheKey := types.Hash(blk) + types.Hash(txInfo)
	if value, ok := s.txTraceCache.Get(cacheKey); ok {
		return value.(*ngy.ExecutionResult), nil
	}

	msg, vmctx, statedb, err := s.ngy.ComputeTxEnv(blk, int(txInfo.txIndex), defaultTraceReExec)
	if err != nil {
		return nil, common.NewError(common.CatchAllError, map[string]interface{}{
			"message": err.Error(),
		})
	}
	execResultInterface, err := s.ngy.TraceTx(ctx, msg, vmctx, statedb, &ngy.TraceConfig{
	...
	execResult, ok := execResultInterface.(*ngy.ExecutionResult)
	if !ok {
		return nil, common.NewError(common.CatchAllError, map[string]interface{}{
...
// getBlock ..
func getBlock(
	ctx context.Context, ngy *ngy.nordicenergy, blockID *types.PartialBlockIdentifier,
) (blk *ngytypes.Block, rosettaError *types.Error) {
	var err error
	if blockID.Hash != nil {
		requestBlockHash := ethcommon.HexToHash(*blockID.Hash)
		blk, err = ngy.GetBlock(ctx, requestBlockHash)
	} else if blockID.Index != nil {
		blk, err = ngy.BlockByNumber(ctx, rpc.BlockNumber(*blockID.Index).EthBlockNumber())
	} else {
