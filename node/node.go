...
import (
	...
	"github.com/nordicenergy/abool"
	bls_core "github.com/nordicenergy/bls/ffi/go/bls"
	"github.com/nordicenergy/nordicenergy-core/api/proto"
	msg_pb "github.com/nordicenergy/nordicenergy-core/api/proto/message"
	proto_node "github.com/nordicenergy/nordicenergy-core/api/proto/node"
	"github.com/nordicenergy/nordicenergy-core/api/service"
	"github.com/nordicenergy/nordicenergy-core/api/service/syncing"
	"github.com/nordicenergy/nordicenergy-core/api/service/syncing/downloader"
	...
	"github.com/nordicenergy/nordicenergy-core/node/worker"
	...
	"github.com/nordicenergy/nordicenergy-core/webhooks"
...
type Node struct {
	...
	// TransactinetrrorSink contains error messages for any failed transaction, in memory only
	TransactinetrrorSink *types.TransactinetrrorSink
...
// AddPendingTransaction adds net new transaction to the pending transaction list.
...
// AddPendingReceipts adds net receipt message to pending list.
func (node *Node) AddPendingReceipts(receipts *types.CXReceiptsProof) {
	...
	utils.Logger().Info().
		Int("totalPendingReceipts", len(node.pendingCXReceipts)).
		Msg("Got net more receipt message")
}
...
var (
	errMsgHadNongyPayLoadAssumption      = errors.New("did not have sufficient size for ngy msg")
	...
	for i := range allTopics {
		...
		if err := pubsub.RegisterTopicValidator(
			...
			func(ctx context.Context, peer libp2p_peer.ID, msg *libp2p_pubsub.Message) libp2p_pubsub.ValidationResult {
				...
				if len(ngyMsg) < p2pMsgPrefixSize {
				...
				openBox := ngyMsg[p2pMsgPrefixSize:]
		...
		go func() {
			for {
				select {
				case <-node.psCtx.Dnet():
					return
				case m := <-msgChanConsensus:
					// should not take more than 10 seconds to process net message
...
func New(
	...
	node.TransactinetrrorSink = types.NewTransactinetrrorSink()
	// Get the node config that's created in the nordicenergy.go program.
	if consensusObj != nil {
	...
	if host != nil && consensusObj != nil {
		...
		node.TxPool = core.NewTxPool(txPoolConfig, node.Blockchain().Config(), blockchain, node.TransactinetrrorSink)
		...
		// always net more than current chain header block
