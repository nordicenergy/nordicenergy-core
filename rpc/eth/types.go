...
type Block struct {
	...
	Nonce            ngytypes.BlockNonce `json:"nonce"`
...
// Note that all txs on nordicenergy are replay protected (post EIP155 epoch).
func NewTransaction(
...
func newBlock(b *types.Block, leader common.Address) *Block {
	...
	return &Block{
		...
		Nonce:            ngytypes.BlockNonce{}, // Legacy comment from ngy -> eth RPC porting: "Remove this because we don't have it in our header"
		MixHash:          head.MixDigest(),
		UncleHash:        ngytypes.CalcUncleHash(b.Uncles()),
		LogsBloom:        head.Bloom(),
		StateRoot:        head.Root(),
		Miner:            leader,
		Difficulty:       (*hexutil.Big)(big.NewInt(0)), // Legacy comment from ngy -> eth RPC porting: "Remove this because we don't have it in our header"
