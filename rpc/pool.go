...
// PublicPoolService provides an API to access the nordicenergy node's transaction pool.
...
func NewPublicPoolAPI(ngy *ngy.nordicenergy, version Version) rpc.API {
	return rpc.API{
		Namespace: version.Namespace(),
		Version:   APIVersion,
		Service:   &PublicPoolService{ngy, version},
		Public:    true,
	}
}
...
func (s *PublicPoolService) SendRawTransaction(
	...
	// Submit transaction
	if err := s.ngy.SendTx(ctx, tx); err != nil {
	...
	// Log submission
	if tx.To() == nil {
		signer := types.MakeSigner(s.ngy.ChainConfig(), s.ngy.CurrentBlock().Epoch())
		ethSigner := types.NewEIP155Signer(s.ngy.ChainConfig().EthCompatibleChainID)
...
func (s *PublicPoolService) verifyChainID(tx *types.Transaction) error {
	nodeChainID := s.ngy.ChainConfig().ChainID
...
func (s *PublicPoolService) SendRawStakingTransaction(
	...
	// Submit transaction
	if err := s.ngy.SendStakingTx(ctx, tx); err != nil {
...
func (s *PublicPoolService) GetPoolStats(
	...
	pendingCount, queuedCount := s.ngy.GetPoolStats()
...
func (s *PublicPoolService) PendingTransactions(
	...
	pending, err := s.ngy.GetPoolTransactions()
	if err != nil {
		return nil, err
	}
...
func (s *PublicPoolService) GetCurrentTransactinetrrorSink(
	ctx context.Context,
) ([]StructuredResponse, error) {
	...
	for _, txError := range s.ngy.GetCurrentTransactinetrrorSink() {
...
func (s *PublicPoolService) GetCurrentStakingErrorSink(
	...
	formattedErrors := []StructuredResponse{}
	for _, txErr := range s.ngy.GetCurrentStakingErrorSink() {
...
func (s *PublicPoolService) GetPendingCXReceipts(
	...
	for _, receipts := range s.ngy.GetPendingCXReceipts() {
