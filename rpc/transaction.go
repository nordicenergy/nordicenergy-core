...
import (
	...
	"github.com/nordicenergy/nordicenergy-core/accounts/abi"
...
// PublicTransactionService provides an API to access nordicenergy's transaction service.
...
func NewPublicTransactionAPI(ngy *ngy.nordicenergy, version Version) rpc.API {
	return rpc.API{
		Namespace: version.Namespace(),
		Version:   APIVersion,
		Service:   &PublicTransactionService{ngy, version},
		Public:    true,
	}
}
...
func (s *PublicTransactionService) GetAccountNonce(
	...
	return s.ngy.GetAccountNonce(ctx, addr, blockNum)
}
...
func (s *PublicTransactionService) GetTransactionCount(
	...
	if blockNum == rpc.PendingBlockNumber {
		...
		nonce, err = s.ngy.GetPoolNonce(ctx, address)
		if err != nil {
			return nil, err
		}
	} else {
...
func (s *PublicTransactionService) GetTransactionsCount(
	...
	if !strings.HasPrefix(address, "net1") {
		// Handle hex address
		addr := internal_common.ParseAddr(address)
	...
	// Response output is the same for all versions
	return s.ngy.GetTransactionsCount(address, txType)
}
...
func (s *PublicTransactionService) GetStakingTransactionsCount(
	...
	// Response output is the same for all versions
	return s.ngy.GetStakingTransactionsCount(address, txType)
}
...
func (s *PublicTransactionService) EstimateGas(
	...
	gas, err := EstimateGas(ctx, s.ngy, args, nil)
	if err != nil {
		return 0, err
	}
...
func (s *PublicTransactionService) GetTransactionByHash(
	...
	tx, blockHash, blockNumber, index := rawdb.ReadTransaction(s.ngy.ChainDb(), hash)
	if tx == nil {
		utils.Logger().Debug().
	...
	block, err := s.ngy.GetBlock(ctx, blockHash)
	if err != nil {
		utils.Logger().Debug().
			Err(err).
			Msgf("%v error at %v", LogTag, "GetTransactionByHash")
...
func (s *PublicTransactionService) GetStakingTransactionByHash(
	...
	// Try to return an already finalized transaction
	stx, blockHash, blockNumber, index := rawdb.ReadStakingTransaction(s.ngy.ChainDb(), hash)
	if stx == nil {
		utils.Logger().Debug().
			Err(errors.Wrapf(ErrTransactionNotFound, "hash %v", hash.String())).
...
func (s *PublicTransactionService) GetTransactionsHistory(
	...
	var err error
	if strings.HasPrefix(args.Address, "net1") {
	...
	hashes, err := s.ngy.GetTransactionsHistory(address, args.TxType, args.Order)
	if err != nil {
		return nil, err
	}
...
func (s *PublicTransactionService) GetStakingTransactionsHistory(
	...
	hashes, err := s.ngy.GetStakingTransactionsHistory(address, args.TxType, args.Order)
	if err != nil {
		utils.Logger().Debug().
			Err(err).
			Msgf("%v error at %v", LogTag, "GetStakingTransactionsHistory")
...
func (s *PublicTransactionService) GetBlockTransactionCountByNumber(
	...
	// Fetch block
	block, err := s.ngy.BlockByNumber(ctx, blockNum)
	if err != nil {
		utils.Logger().Debug().
			Err(err).
...
func (s *PublicTransactionService) GetTransactionReceipt(
	...
	if tx == nil {
		...
		if block, _ := s.ngy.GetBlock(ctx, blockHash); block != nil {
			index = index + uint64(block.Transactions().Len())
		}
	}
	receipts, err := s.ngy.GetReceipts(ctx, blockHash)
	if err != nil {
		return nil, err
	}
...
func (s *PublicTransactionService) GetCXReceiptByHash(
	...
	if cx, blockHash, blockNumber, _ := rawdb.ReadCXReceipt(s.ngy.ChainDb(), hash); cx != nil {
		// Format response according to version
		switch s.version {
		case V1, Eth:
...
func (s *PublicTransactionService) ResendCx(ctx context.Context, txID common.Hash) (bool, error) {
	_, success := s.ngy.ResendCx(ctx, txID)
...
func EstimateGas(ctx context.Context, ngy *ngy.nordicenergy, args CallArgs, gasCap *big.Int) (uint64, error) {
	...
	executable := func(gas uint64) (bool, *core.ExecutionResult, error) {
		...
		result, err := DoEVMCall(ctx, ngy, args, blockNum, 0)
		if err != nil {
	...
	// Execute the binary search and hnet in on an executable gas limit
	for lo+1 < hi {
		mid := (hi + lo) / 2
