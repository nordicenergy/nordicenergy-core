...
// PublicBlockchainService provides an API to access the nordicenergy blockchain.
...
func NewPublicBlockchainAPI(ngy *ngy.nordicenergy, version Version) rpc.API {
	return rpc.API{
		Namespace: version.Namespace(),
		Version:   APIVersion,
		Service:   &PublicBlockchainService{ngy, version},
		Public:    true,
	}
}
...
func (s *PublicBlockchainService) ChainId(ctx context.Context) (interface{}, error) {
	...
	switch s.version {
	case V1:
		return hexutil.Uint64(s.ngy.ChainID), nil
	case V2:
		return s.ngy.ChainID, nil
	case Eth:
...
// Instead, users should send already signed raw transactions using ngy_sendRawTransaction or eth_sendRawTransaction
...
func (s *PublicBlockchainService) BlockNumber(ctx context.Context) (interface{}, error) {
	// Fetch latest header
	header, err := s.ngy.HeaderByNumber(ctx, rpc.LatestBlockNumber)
	if err != nil {
		return nil, err
	}
...
// When withSigners in BlocksArgs is true it shows block signers for this block in list of net addresses.
func (s *PublicBlockchainService) GetBlockByNumber(
	...
	// Disable isBlockGreaterThanLatest checks for Ethereum RPC:s, but keep them in place for legacy ngy_ RPC:s for now to ensure backwards compatibility
	if s.version != Eth && isBlockGreaterThanLatest(s.ngy, blockNum) {
	...
	if blk != nil && err == nil {
		...
		leader := s.ngy.GetLeaderAddress(blk.Header().Coinbase(), blk.Header().Epoch())
...
// it shows block signers for this block in list of net addresses.
func (s *PublicBlockchainService) GetBlockByHash(
	...
	// Fetch the block
	blk, err := s.ngy.GetBlock(ctx, blockHash)
	if blk != nil && err == nil {
		if blockArgs.WithSigners {
...
func (s *PublicBlockchainService) GetBlocks(
	...
	for i := blockStart; i <= blockEnd; i++ {
		...
		if blockNum.Int64() > s.ngy.CurrentBlock().Number().Int64() {
			break
		}
...
func (s *PublicBlockchainService) GetBlockSigners(
	...
	if blockNum.Int64() == 0 || blockNum.Int64() >= s.ngy.CurrentBlock().Number().Int64() {
		return []string{}, nil
	}
	...
	// Fetch signers
	slots, mask, err := s.ngy.GetBlockSigners(ctx, blockNum)
	if err != nil {
		return nil, err
	}
	...
	for _, validator := range slots {
		...
		if ok, err := mask.KeyEnabled(validator.BLSPublicKey); err == nil && ok {
			signers = append(signers, netAddress)
		}
	}
	return signers, nil
}
...
func (s *PublicBlockchainService) GetSignedBlocks(
	...
	lastBlock := uint64(0)
	blockHeight := s.ngy.CurrentBlock().Number().Uint64()
	instance := shard.Schedule.InstanceForEpoch(s.ngy.CurrentBlock().Epoch())
...
func (s *PublicBlockchainService) GetLeader(ctx context.Context) (string, error) {
	...
	// Response output is the same for all versions
	leader := s.ngy.GetLeaderAddress(header.Coinbase(), header.Epoch())
	return leader, nil
}
...
func (s *PublicBlockchainService) GetShardingStructure(
	...
	epoch := s.ngy.CurrentBlock().Epoch()
...
func (s *PublicBlockchainService) GetLatestChainHeaders(
	...
	// Response output is the same for all versions
	return NewStructuredResponse(s.ngy.GetLatestChainHeaders())
}
...
func (s *PublicBlockchainService) GetLastCrossLinks(
	...
	// Fetch crosslinks
	crossLinks, err := s.ngy.GetLastCrossLinks()
	if err != nil {
		return nil, err
	}
...
func (s *PublicBlockchainService) GetHeaderByNumber(
	...
	// Fetch Header
	header, err := s.ngy.HeaderByNumber(ctx, blockNum)
...
func (s *PublicBlockchainService) GetCurrentUtilityMetrics(
	...
	// Fetch metrics
	metrics, err := s.ngy.GetCurrentUtilityMetrics()
	if err != nil {
		return nil, err
	}
...
func (s *PublicBlockchainService) GetSuperCommittees(
	...
	cmt, err := s.ngy.GetSuperCommittees()
	if err != nil {
		return nil, err
	}
...
func (s *PublicBlockchainService) GetCurrentBadBlocks(
	...
	for _, blk := range s.ngy.GetCurrentBadBlocks() {
		// Response output is the same for all versions
...
func (s *PublicBlockchainService) GetTotalSupply(
	...
	return stakingReward.GetTotalTokens(s.ngy.BlockChain)
}
...
func (s *PublicBlockchainService) GetCirculatingSupply(
	...
	return chain.GetCirculatingSupply(ctx, s.ngy.BlockChain)
}
...
func (s *PublicBlockchainService) GetStakingNetworkInfo(
	...
	totalStaking := s.ngy.GetTotalStakingSnapshot()
	header, err := s.ngy.HeaderByNumber(ctx, rpc.LatestBlockNumber)
	if err != nil {
		return nil, err
	}
	medianSnapshot, err := s.ngy.GetMedianRawStakeSnapshot()
	if err != nil {
		return nil, err
	}
...
func (s *PublicBlockchainService) InSync(ctx context.Context) (bool, error) {
	return !s.ngy.NodeAPI.IsOutOfSync(s.ngy.BlockChain), nil
}
...
func (s *PublicBlockchainService) BeaconInSync(ctx context.Context) (bool, error) {
	return !s.ngy.NodeAPI.IsOutOfSync(s.ngy.BeaconChain), nil
}

func isBlockGreaterThanLatest(ngy *ngy.nordicenergy, blockNum rpc.BlockNumber) bool {
	...
	return uint64(blockNum) > ngy.CurrentBlock().NumberU64()
}
