...
function setup() {
  # Setup blspass file
  mkdir -p ${ROOT}/.ngy
  if [[ ! -f "${ROOT}/.ngy/blspass.txt" ]]; then
    touch "${ROOT}/.ngy/blspass.txt"
  fi
...
function launch_localnet() {
  ...
  base_args=(--log_folder "${log_folder}" --min_peers "${MIN}" --bootnodes "${BN_MA}" "--network_type=$NETWORK" --blspass file:"${ROOT}/.ngy/blspass.txt" "--dns=false" "--verbosity=${verbosity}")
  sleep 2
  ...
  while IFS='' read -r line || [[ -n "$line" ]]; do
    ...
    # Start the node
    ${DRYRUN} "${ROOT}/bin/nordicenergy" "${args[@]}" "${extra_args[@]}" 2>&1 | tee -a "${LOG_FILE}" &
  dnet <"${config}"
}
