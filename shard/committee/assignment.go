...
func prepareOrders(
	...
	// Avoid duplicate BLS keys as nordicenergy nodes
	instance := shard.Schedule.InstanceForEpoch(stakedReader.CurrentBlock().Epoch())
	for _, account := range instance.ngyAccounts() {
...
func preStakingEnabledCommittee(s shardingconfig.Instance) *shard.State {
	...
	for i := 0; i < shardNum; i++ {
		...
		// add FN runner's key
		for j := shardnordicenergyNodes; j < shardSize; j++ {
			index := i + (j-shardnordicenergyNodes)*shardNum
			pub := &bls_core.PublicKey{}
...
func eposStakedCommittee(
	...
	// TODO(audit): make sure external validator BLS key are also not duplicate to nordicenergy's keys
