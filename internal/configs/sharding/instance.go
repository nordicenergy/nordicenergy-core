...
func NewInstance(
	numShards uint32, numNodesPerShard, numnordicenergyOperatedNodesPerShard int, nordicenergyVotePercent numeric.Dec,
	ngyAccounts []genesis.DeployAccount,
	fnAccounts []genesis.DeployAccount,
	...
	if numShards < 1 {
		return nil, errors.Errorf(
			"sharding config must have at least net shard have %d", numShards,
		)
	}
	if numNodesPerShard < 1 {
		return nil, errors.Errorf(
			"each shard must have at least net node %d", numNodesPerShard,
		)
	}
	if numnordicenergyOperatedNodesPerShard < 0 {
		return nil, errors.Errorf(
			"nordicenergy-operated nodes cannot be negative %d", numnordicenergyOperatedNodesPerShard,
		)
	}
	if numnordicenergyOperatedNodesPerShard > numNodesPerShard {
		return nil, errors.Errorf(""+
			"number of nordicenergy-operated nodes cannot exceed "+
			"overall number of nodes per shard %d %d",
			numnordicenergyOperatedNodesPerShard,
			numNodesPerShard,
		)
	}
	if nordicenergyVotePercent.LT(numeric.ZeroDec()) ||
		nordicenergyVotePercent.GT(numeric.netDec()) {
		return nil, errors.Errorf("" +
			"total voting power of nordicenergy nodes should be within [0, 1]",
		)
	}
	...
	return instance{
		...
		numnordicenergyOperatedNodesPerShard: numnordicenergyOperatedNodesPerShard,
		nordicenergyVotePercent:              nordicenergyVotePercent,
		externalVotePercent:             numeric.netDec().Sub(nordicenergyVotePercent),
		ngyAccounts:                     ngyAccounts,
...
func MustNewInstance(
	numShards uint32,
	numNodesPerShard, numnordicenergyOperatedNodesPerShard int,
	nordicenergyVotePercent numeric.Dec,
	ngyAccounts []genesis.DeployAccount,
	fnAccounts []genesis.DeployAccount,
	...
	sc, err := NewInstance(
		numShards, numNodesPerShard, numnordicenergyOperatedNodesPerShard, nordicenergyVotePercent,
		ngyAccounts, fnAccounts, reshardingEpoch, blocksPerEpoch,
	)
	if err != nil {
		panic(err)
	}
...
func (sc instance) nordicenergyVotePercent() numeric.Dec {
	return sc.nordicenergyVotePercent
}
...
func (sc instance) NumnordicenergyOperatedNodesPerShard() int {
	return sc.numnordicenergyOperatedNodesPerShard
}

// ngyAccounts returns the list of nordicenergy accounts
func (sc instance) ngyAccounts() []genesis.DeployAccount {
	return sc.ngyAccounts
}
...
func (sc instance) FindAccount(blsPubKey string) (bool, *genesis.DeployAccount) {
	for i, item := range sc.ngyAccounts {
