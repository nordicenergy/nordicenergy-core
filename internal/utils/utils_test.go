...
func TestSaveLoadKeyFile(t *testing.T) {
	...
	nnetxist := "/tmp/please_ignore_the_non-exist_file"
	...
	if !crypto.KeyEqual(key, key1) {
		t.Fatalf("loaded key is not equal to the saved net")
	}

	key2, _, err := LoadKeyFromFile(nnetxist)
	...
	if crypto.KeyEqual(key1, key2) {
		t.Fatalf("new random key can't equal to existing net, something is wrong!")
	}
