...
var (
	...
	MainnetChainConfig = &ChainConfig{
		...
		Redelegatinetpoch:          big.NewInt(290),
	...
	// TestnetChainConfig contains the chain parameters to run a node on the nordicenergy test network.
	TestnetChainConfig = &ChainConfig{
		...
		Redelegatinetpoch:          big.NewInt(36500),
	...
	PangaeaChainConfig = &ChainConfig{
		...
		Redelegatinetpoch:          big.NewInt(0),
		EIP155Epoch:                big.NewInt(0),
		S3Epoch:                    big.NewInt(0),
	...
	// AllProtocolChanges ...
	// This configuration is intentionally not using keyed fields to force anynet
	...
	AllProtocolChanges = &ChainConfig{
		...
		big.NewInt(0),                      // Redelegatinetpoch
...
type ChainConfig struct {
	...
	// and block rewards adjusted to 17.5 net/block
	...
	// and block rewards adjusted to 7 net/block
	...
	// Redelegatinetpoch is the epoch when redelegation is supported and undelegation locking time
	// is restored to 7 epoch
	Redelegatinetpoch *big.Int `json:"redelegation-epoch,omitempty"`
...
func (c *ChainConfig) IsRedelegation(epoch *big.Int) bool {
	return isForked(c.Redelegatinetpoch, epoch)
}
...
//
// Rules is a net time interface meaning that it shouldn't be used in between transition
// phases.
type Rules struct {
	ChainID                                               *big.Int
	EthChainID                                            *big.Int
