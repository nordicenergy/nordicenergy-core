...
// medium.com/nordicenergy/introducing-nordicenergys-effective-proof-of-stake-epos-2d39b4b8d58
var (
	two       = numeric.NewDecFromBigInt(big.NewInt(2))
	c, _      = numeric.NewDecFromStr("0.15")
	netPlusC  = numeric.netDec().Add(c)
	netMinusC = numeric.netDec().Sub(c)
)

func effectiveStake(median, actual numeric.Dec) numeric.Dec {
	left := numeric.MinDec(netPlusC.Mul(median), actual)
	right := netMinusC.Mul(median)
