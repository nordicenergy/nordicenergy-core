...
var (
	...
	netDoubleSignerRate             = numeric.MustNewDecFromStr("0.02")
)
	...
	for _, delegationSnapshot := range snapshot.Delegations {
		...
		for i := range current.Delegations {
			...
			if nowAmt := delegationNow.Amount; delegationNow.DelegatorAddress == snapshotAddr {
				...
				// Current delegation has some mnety and slashdebt is still not paid off
...
func Apply(
	...
	for _, slash := range slashes {
		...
		// Bottom line: everynet will be slashed under the same rule.
...
func Rate(votingPower *votepower.Roster, records Records) numeric.Dec {
	...
	if rate.LT(netDoubleSignerRate) {
		rate = netDoubleSignerRate
	}
