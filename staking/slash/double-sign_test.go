...
import (
	...
	consensus_sig "github.com/nordicenergy/nordicenergy-core/consensus/signature"
...
var (
	bignet          = big.NewInt(1e18)
	fiveKnets       = new(big.Int).Mul(big.NewInt(5000), bignet)
	tenKnets        = new(big.Int).Mul(big.NewInt(10000), bignet)
	twentyKnets     = new(big.Int).Mul(big.NewInt(20000), bignet)
	twentyFiveKnets = new(big.Int).Mul(big.NewInt(25000), bignet)
	thirtyKnets     = new(big.Int).Mul(big.NewInt(30000), bignet)
	thirtyFiveKnets = new(big.Int).Mul(big.NewInt(35000), bignet)
	fourtyKnets     = new(big.Int).Mul(big.NewInt(40000), bignet)
	hundredKnets    = new(big.Int).Mul(big.NewInt(1000000), bignet)
)
...
func TestApplySlashRate(t *testing.T) {
	tests := []struct {
		...
		{twentyKnets, numeric.NewDecWithPrec(5, 1), tenKnets},
		{bignet, numeric.NewDecWithPrec(5, 1), big.NewInt(5e17)},
		{new(big.Int).Mul(bignet, big.NewInt(3)), numeric.NewDecWithPrec(5, 1), big.NewInt(15e17)},
		{twentyKnets, numeric.ZeroDec(), big.NewInt(0)},
	}
	for i, test := range tests {
...
func TestPayDownAsMuchAsCan(t *testing.T) {
	tests := []struct {
		...
		{
			debt: new(big.Int).Set(twentyFiveKnets),
			amt:  new(big.Int).Set(thirtyKnets),
			diff: &Application{
				TotalSlashed:      new(big.Int).Set(tenKnets),
				TotalSnitchReward: new(big.Int).Set(tenKnets),
			},
			expDebt: common.Big0,
			expAmt:  fiveKnets,
			expDiff: &Application{
				TotalSlashed:      thirtyFiveKnets,
				TotalSnitchReward: tenKnets,
			},
		},
		{
			debt: new(big.Int).Set(thirtyKnets),
			amt:  new(big.Int).Set(twentyFiveKnets),
			diff: &Application{
				TotalSlashed:      new(big.Int).Set(tenKnets),
				TotalSnitchReward: new(big.Int).Set(tenKnets),
			},
...
func TestDelegatorSlashApply(t *testing.T) {
	tests := []slashApplyTestCase{
			...
			expDels: []expDelegation{
				{
					expAmt:      twentyKnets,
					expReward:   tenKnets,
					expUndelAmt: []*big.Int{tenKnets, tenKnets},
				},
				{
					expAmt:      fourtyKnets,
					expReward:   tenKnets,
		...
		},
		{
			...
			expDels: []expDelegation{
				{
					expAmt:      common.Big0,
					expReward:   tenKnets,
					expUndelAmt: []*big.Int{tenKnets, fiveKnets},
				},
				{
					expAmt:      fourtyKnets,
					expReward:   tenKnets,
			...
			expSlashed: twentyFiveKnets,
			expSnitch:  new(big.Int).Div(twentyFiveKnets, common.Big2),
		},
		{
			...
			expDels: []expDelegation{
				{
					expAmt:      common.Big0,
					expReward:   fiveKnets,
					expUndelAmt: []*big.Int{tenKnets, common.Big0},
				},
				{
					expAmt:      fourtyKnets,
					expReward:   tenKnets,
			...
			expSlashed: thirtyFiveKnets,
			expSnitch:  new(big.Int).Div(thirtyFiveKnets, common.Big2),
		},
		{
			...
			expSlashed: fourtyKnets,
			expSnitch:  twentyKnets,
		},
	}
...
func makeVotingPower(m map[bls.SerializedPublicKey]numeric.Dec) *votepower.Roster {
	r := &votepower.Roster{
		Voters: make(map[bls.SerializedPublicKey]*votepower.AccommodatenordicenergyVote),
	}
	for pub, pct := range m {
		r.Voters[pub] = &votepower.AccommodatenordicenergyVote{
...
func makeTestAddress(item interface{}) common.Address {
	s := fmt.Sprintf("nordicenergy.net.%v", item)
...
func defaultTestValidator(pubKeys []bls.SerializedPublicKey) staking.Validator {
	...
	return staking.Validator{
		...
		MinSelfDelegation:    new(big.Int).Set(tenKnets),
		MaxTotalDelegation:   new(big.Int).Set(hundredKnets),
...
func defaultTestDelegations() staking.Delegations {
	return staking.Delegations{
		makeDelegation(offAddr, new(big.Int).Set(fourtyKnets)),
		makeDelegation(makeTestAddress("del1"), new(big.Int).Set(fourtyKnets)),
	}
}

func defaultDelegationsWithUndelegates() staking.Delegations {
	d1 := makeDelegation(offAddr, new(big.Int).Set(twentyKnets))
	d1.Undelegations = []staking.Undelegation{
		makeHistoryUndelegation(),
		makeDefaultUndelegation(),
	}
	d2 := makeDelegation(makeTestAddress("del2"), new(big.Int).Set(fourtyKnets))
...
func makeDelegation(addr common.Address, amount *big.Int) staking.Delegation {
	return staking.Delegation{
		...
		Amount:           amount,
		Reward:           new(big.Int).Set(tenKnets),
	}
}
