...
func TestIncrementValidatorSigningCounts(t *testing.T) {
	...
	for _, test := range tests {
		ctx, err := makeIncStateTestCtx(test.numngySlots, test.numUserSlots, test.verified)
		if err != nil {
			t.Fatal(err)
		}
...
type incStateTestCtx struct {
	...
	//  typeIncngyNode - 2: keep the code field unchanged
...
func makeIncStateTestCtx(numngySlots, numUserSlots int, verified []int) (*incStateTestCtx, error) {
	cmt := makeTestMixedCommittee(numngySlots, numUserSlots, 0)
	staked := cmt.StakedValidators()
	bitmap, _ := indexesToBitMap(verified, numUserSlots+numngySlots)
...
func (ctx *incStateTestCtx) computeSlotMaps() {
	...
	for _, slot := range ctx.cmt.Slots {
		if slot.EffectiveStake == nil {
			ctx.computedSlotMap[slot.EcdsaAddress] = typeIncngyNode
		}
	}
}
...
func (ctx *incStateTestCtx) checkAddrIncStateByType(addr common.Address, typeInc int) error {
	...
	case typeIncngyNode:
		if err = ctx.checkngyNodeStateChangeByAddr(addr); err != nil {
			err = fmt.Errorf("nordicenergy node address %s: %v", addr, err)
		}
	default:
...
// checkngyNodeStateChangeByAddr checks the state change for ngy nodes. Since ngy nodes does not
// have wrapper, it is supposed to be unchanged in code field
func (ctx *incStateTestCtx) checkngyNodeStateChangeByAddr(addr common.Address) error {
...
func makeTestCommittee(n int, shardID uint32) *shard.Committee {
	...
	for i := 0; i != n; i++ {
		slots = append(slots, makengySlot(i, shardID))
	}
...
func makengySlot(seed int, shardID uint32) shard.Slot {
...
// makeTestMixedCommittee makes a committee with both nordicenergy nodes and user nodes
func makeTestMixedCommittee(numngySlots, numUserSlots int, shardID uint32) *shard.Committee {
	slots := make(shard.SlotList, 0, numngySlots+numUserSlots)
	for i := 0; i != numngySlots; i++ {
		slots = append(slots, makengySlot(i, shardID))
	}
	for i := numngySlots; i != numngySlots+numUserSlots; i++ {
