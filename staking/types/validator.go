...
const (
	...
	BLSVerificationStr       = "nordicenergy"
	TenThousand              = 10000
	APRHistoryLength         = 30
...
var (
	...
	errMinSelfDelegationTooSmall = errors.New(
		"min_self_delegation must be greater than or equal to 10,000 net",
	)
	...
	errNeedAtLeastnetSlotKey = errors.New("need at least net slot key")
...
var (
	netAsBigInt  = big.NewInt(denominations.net)
	minimumStake = new(big.Int).Mul(netAsBigInt, big.NewInt(TenThousand))
)
...
func (v *Validator) SanityCheck() error {
	...
	if len(v.SlotPubKeys) == 0 {
		return errNeedAtLeastnetSlotKey
	}
	...
	// MinSelfDelegation must be >= 10000 net
...
func containsnordicenergyBLSKeys(
	blsKeys []bls.SerializedPublicKey,
	ngyAccounts []genesis.DeployAccount,
	epoch *big.Int,
) error {
	for i := range blsKeys {
		if err := matchesnordicenergyBLSKey(
			&blsKeys[i], ngyAccounts, epoch,
		); err != nil {
			return err
		}
	}
...
func matchesnordicenergyBLSKey(
	...
	return func() error {
		...
		if _, ok := cache[key]; !ok {
			// net time cost per epoch
			cache[key] = map[publicKeyAsHex]struct{}{}
			for i := range ngyAccounts {
				// invariant assume it is hex
				cache[key][ngyAccounts[i].BLSPublicKey] = struct{}{}
			}
		}
...
func CreateValidatorFromNewMsg(
	...
	if err := containsnordicenergyBLSKeys(
		pubKeys, instance.ngyAccounts(), epoch,
	); err != nil {
		return nil, err
	}
...
func UpdateValidatorFromEditMsg(validator *Validator, edit *EditValidator, epoch *big.Int) error {
	...
	if edit.SlotKeyToAdd != nil {
		...
		if !found {
			instance := shard.Schedule.InstanceForEpoch(epoch)
			if err := matchesnordicenergyBLSKey(
				edit.SlotKeyToAdd, instance.ngyAccounts(), epoch,
			); err != nil {
				return err
			}
