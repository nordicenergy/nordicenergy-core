...
var (
	...
	ngyBLSPub      bls.SerializedPublicKey

	ngyBLSPubStr     = "c2962419d9999a87daa134f6d177f9ccabfe168a470587b13dd02ce91d1690a92170e5949d3dbdfc1b13fd7327dbef8c"
	validatorAddr, _ = common2.Bech32ToAddress("net1pdv9lrdwl0rg5vglh4xtyrv3wjk3wsqket7zxy")
)
...
var (
	validDescription = Description{
		Name:            "Jacky Wang",
		Identity:        "jacky@nordicenergy.net",
		Website:         "nordicenergy.net/jacky",
		SecurityContact: "jacky@nordicenergy.net",
...
func init() {
	// set bls pub keys for ngy
	copy(ngyBLSPub[:], common.Hex2Bytes(ngyBLSPubStr))
}
...
func TestValidator_SanityCheck(t *testing.T) {
	tests := []struct {
		...
		{
			func(v *Validator) { v.Rate, v.MaxRate = netRate, halfRate },
			errCommissionRateTooLarge,
		},
		{
			func(v *Validator) { v.MaxChangeRate, v.MaxRate = netRate, halfRate },
			errCommissionRateTooLarge,
		},
		{
			func(v *Validator) {
...
func TestUpdateDescription(t *testing.T) {
	...
	for i, test := range tests {
		...
		if err := assertDescriptinetqual(d, test.expect); err != nil {
...
func TestDescription_EnsureLength(t *testing.T) {
	...
	for i, test := range tests {
		...
		if err := assertDescriptinetqual(test.desc, d); err != nil {
...
func TestContainsnordicenergyBLSKeys(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := containsnordicenergyBLSKeys(pubs, das, big.NewInt(0))
		if assErr := assertError(err, test.expErr); assErr != nil {
			t.Errorf("Test %v: %v", i, assErr)
		}
	}
...
func TestCreateValidatorFromNewMsg(t *testing.T) {
	tests := []struct {
		...
		{func(cv *CreateValidator) { cv.SlotPubKeys[0] = ngyBLSPub }, errDuplicateSlotKeys},
	...
	tests := []struct {
		...
		},
		{
			...
			editValidator: EditValidator{
				...
				Description:      Description{Name: "jacky@nordicenergy.net"},
			},
			editExpValidator: func(v *Validator) { v.Name = "jacky@nordicenergy.net" },
		},
		{
		...
		{
			// add pub collide with ngy bls account
			editValidator: EditValidator{
				ValidatorAddress: validatorAddr,
				SlotKeyToAdd:     &ngyBLSPub,
			},
...
func assertValidatorEqual(v1, v2 Validator) error {
	...
	if err := assertDescriptinetqual(v1.Description, v2.Description); err != nil {
...
func assertValidatorAlignCreateValidator(v Validator, cv CreateValidator) error {
	...
	if err := assertDescriptinetqual(v.Description, cv.Description); err != nil {
...
func assertDescriptinetqual(d1, d2 Description) error {
	if d1.Name != d2.Name {
