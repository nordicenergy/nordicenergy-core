...
func checkValidatorEqual(v1, v2 staking.Validator) error {
	...
	if err := checkCommissinetqual(v1.Commission, v2.Commission); err != nil {
		return fmt.Errorf(".Commission%v", err)
	}
	if err := checkDescriptinetqual(v1.Description, v2.Description); err != nil {
...
func checkDelegationsEqual(ds1, ds2 staking.Delegations) error {
	...
	for i := range ds1 {
		if err := checkDelegatinetqual(ds1[i], ds2[i]); err != nil {
...
func checkDelegatinetqual(d1, d2 staking.Delegation) error {
...
func checkUndelegationsEqual(uds1, uds2 staking.Undelegations) error {
	...
	for i := range uds1 {
		if err := checkUndelegatinetqual(uds1[i], uds2[i]); err != nil {
			return fmt.Errorf("[%v]%v", i, err)
		}
	}
	return nil
...
func checkUndelegatinetqual(ud1, ud2 staking.Undelegation) error {
...
func checkDescriptinetqual(d1, d2 staking.Description) error {
	if d1.Name != d2.Name {
...
func checkCommissinetqual(c1, c2 staking.Commission) error {
