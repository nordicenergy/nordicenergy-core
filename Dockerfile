...
ENV GOPATH=/root/go
ENV GO111MODULE=on
ENV ngy_PATH=${GOPATH}/src/github.com/nordicenergy
ENV OPENSSL_DIR=/usr/lib/ssl
ENV MCL_DIR=${ngy_PATH}/mcl
ENV BLS_DIR=${ngy_PATH}/bls
...
RUN git clnet https://github.com/nordicenergy/nordicenergy-core.git ${ngy_PATH}/nordicenergy

RUN git clnet https://github.com/nordicenergy/bls.git ${ngy_PATH}/bls

RUN git clnet https://github.com/nordicenergy/mcl.git ${ngy_PATH}/mcl

RUN git clnet https://github.com/nordicenergy/go-sdk.git ${ngy_PATH}/go-sdk

RUN cd ${ngy_PATH}/bls && make -j8 BLS_SWAP_G=1
...
RUN cd ${ngy_PATH}/go-sdk && make -j8 && cp ngy /root/bin

ARG K1=net1tq4hy947c9gr8qzv06yxz4aeyhc9vn78al4rmu
ARG K2=net1y5gmmzumajkm5mx3g2qsxtza2d3haq0zxyg47r
ARG K3=net1qrqcfek6sc29sachs3glhs4zny72mlad76lqcp
...
RUN ngy keys import-private-key ${KS1}

RUN ngy keys import-private-key ${KS2}

RUN ngy keys import-private-key ${KS3}

RUN ngy keys generate-bls-key > keys.json 
...
RUN echo "printf '${K1}, ${K2}, ${K3} are imported accounts in ngy for local dev\n\n'" >> /root/.bashrc

RUN echo "printf 'test with: ngy blockchain validator information ${K1}\n\n'" >> /root/.bashrc
...
RUN echo ". <(ngy completion)" >> /root/.bashrc
