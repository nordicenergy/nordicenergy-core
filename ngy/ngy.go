...
import (
	...
	"github.com/nordicenergy/nordicenergy-core/api/proto"
...
// nordicenergy implements the nordicenergy full node service.
type nordicenergy struct {
	// Channel for shutting down the service
	ShutdownChan  chan bool                      // Channel for shutting down the nordicenergy
...
type NodeAPI interface {
	...
	ReportStakingErrorSink() types.TransactinetrrorReports
	ReportPlainErrorSink() types.TransactinetrrorReports
...
// New creates a new nordicenergy object (including the
// initialisation of the common nordicenergy object)
func New(
...
// SingleFlightRequest ..
func (ngy *nordicenergy) SingleFlightRequest(
	key string,
	fn func() (interface{}, error),
) (interface{}, error) {
	res, err, _ := ngy.group.Do(key, fn)
	return res, err
}

// SingleFlightForgetKey ...
func (ngy *nordicenergy) SingleFlightForgetKey(key string) {
	ngy.group.Forget(key)
}

// ProtocolVersion ...
func (ngy *nordicenergy) ProtocolVersion() int {
	return proto.ProtocolVersion
}

// IsLeader exposes if node is currently leader
func (ngy *nordicenergy) IsLeader() bool {
	return ngy.NodeAPI.IsCurrentlyLeader()
}

// GetNodeMetadata ..
func (ngy *nordicenergy) GetNodeMetadata() commonRPC.NodeMetadata {
	header := ngy.CurrentBlock().Header()
	...
	c := commonRPC.C{}
	c.TotalKnownPeers, c.Connected, c.NotConnected = ngy.NodeAPI.PeerConnectivity()

	consensusInternal := ngy.NodeAPI.GetConsensusInternal()

	return commonRPC.NodeMetadata{
		BLSPublicKey:   blsKeys,
		Version:        nodeconfig.GetVersion(),
		NetworkType:    string(cfg.GetNetworkType()),
		ChainConfig:    *ngy.ChainConfig(),
		IsLeader:       ngy.IsLeader(),
		ShardID:        ngy.ShardID,
		...
		NodeBootTime:   ngy.NodeAPI.GetNodeBootTime(),
...
func (ngy *nordicenergy) GetEVM(ctx context.Context, msg core.Message, state *state.DB, header *block.Header) (*vm.EVM, error) {
	state.SetBalance(msg.From(), math.MaxBig256)
	vmCtx := core.NewEVMContext(msg, header, ngy.BlockChain, nil)
	return vm.NewEVM(vmCtx, state, ngy.BlockChain.Config(), *ngy.BlockChain.GetVMConfig()), nil
}

// ChainDb ..
func (ngy *nordicenergy) ChainDb() ethdb.Database {
	return ngy.chainDb
}

// EventMux ..
func (ngy *nordicenergy) EventMux() *event.TypeMux {
	return ngy.eventMux
}

// BloomStatus ...
// TODO: this is not implemented or verified yet for nordicenergy.
func (ngy *nordicenergy) BloomStatus() (uint64, uint64) {
	sections, _, _ := ngy.BloomIndexer.Sections()
