...
func (ngy *nordicenergy) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-ngy.ShutdownChan:
					return

				case request := <-ngy.BloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(ngy.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(ngy.chainDb, task.Bit, section, head); err == nil {
