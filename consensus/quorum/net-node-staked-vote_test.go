...
// 50 nordicenergy Nodes, 50 Staked Nodes
func setupBaseCase() (Decider, *TallyResult, shard.SlotList, map[string]secretKeyMap) {
	...
	for i := 0; i < quorumNodes; i++ {
		...
		if i < 50 {
			...
			sKeys[ngy][newSlot.BLSPublicKey] = sKey
		} else {
...
// 33 nordicenergy Nodes, 67 Staked Nodes
...
func TestEvenNodes(t *testing.T) {
	...
	// Check nordicenergyPercent + StakePercent == 1
	sum := result.ourPercent.Add(result.theirPercent)
	if !sum.Equal(numeric.netDec()) {
		t.Errorf("Total voting power does not equal 1. nordicenergy voting power: %s, Staked voting power: %s, Sum: %s",
	...
	// Sign all nordicenergy Nodes
	sign(stakedVote, sKeys[ngy], Prepare)
	achieved = stakedVote.IsQuorumAchieved(Prepare)
	if !achieved {
	...
	// Commit
	sign(stakedVote, sKeys[ngy], Commit)
	achieved = stakedVote.IsQuorumAchieved(Commit)
	if !achieved {
		t.Errorf("[IsQuorumAchieved] Phase: %s, QuorumAchieved: %s, Expected: true (All nodes = 100%%)",
			Commit, strconv.FormatBool(achieved))
	...
	// ViewChange
	sign(stakedVote, sKeys[ngy], ViewChange)
	achieved = stakedVote.IsQuorumAchieved(ViewChange)
	if !achieved {
...
func Test33nordicenergyNodes(t *testing.T) {
	...
	// Sign all nordicenergy Nodes, 0 Staker Nodes
	// Prepare
	sign(stakedVote, sKeys, Prepare)
	achieved := stakedVote.IsQuorumAchieved(Prepare)
	if !achieved {
		t.Errorf("[IsQuorumAchieved] Phase: %s, QuorumAchieved: %s, Expected: true (All nordicenergy nodes = 68%%)",
			Prepare, strconv.FormatBool(achieved))
	}
	// Commit
	...
	achieved = stakedVote.IsQuorumAchieved(ViewChange)
	if !achieved {
		t.Errorf("[IsQuorumAchieved] Phase: %s, Got: %s, Expected: true (All nordicenergy nodes = 68%%)",
			ViewChange, strconv.FormatBool(achieved))
	}
	// RewardThreshold
	rewarded := stakedVote.IsAllSigsCollected()
	if rewarded {
		t.Errorf("[IsAllSigsCollected] Got: %s, Expected: false (All nordicenergy nodes = 68%%)",
			strconv.FormatBool(rewarded))
	}
}
