...
type ParticipantTracker interface {
	...
	NthNextngy(shardingconfig.Instance, *bls.PublicKeyWrapper, int) (bool, *bls.PublicKeyWrapper)
...
// NthNextngy return the Nth next pubkey of nordicenergy nodes, next can be negative number
func (s *cIdentities) NthNextngy(instance shardingconfig.Instance, pubKey *bls.PublicKeyWrapper, next int) (bool, *bls.PublicKeyWrapper) {
	found := false
	...
	numNodes := instance.NumnordicenergyOperatedNodesPerShard()
	// sanity check to avoid out of bound access
	if numNodes <= 0 || numNodes > len(s.publicKeys) {
		numNodes = len(s.publicKeys)
	}
