...
func (v *stakedVoteWeight) AddNewVote(
	...
	for i, pubKey := range pubKeys {
		...
		if i == 0 {
			...
			// nordicenergy nodes are excluded from this rule.
			if bytes.Compare(signerAddr.Bytes(), voter.EarningAccount[:]) != 0 && !voter.IsnordicenergyNode {
...
func (v *stakedVoteWeight) MarshalJSON() ([]byte, error) {
	...
	type u struct {
		Isnordicenergy      bool   `json:"is-nordicenergy-slot"`
	...
	type t struct {
		...
		ngyVotingPower      string `json:"ngy-voting-power"`
