...
import (
	...
	bls_cosi "github.com/nordicenergy/nordicenergy-core/crypto/bls"
...
type viewChange struct {
	...
	// until net of the viewID has enough votes (>=2f+1)
	// after net of viewID has enough votes, we can reset and clean the map
	// hnetst nodes will never double votes on different viewID
	...
	m1Payload []byte // message payload for type m1 := |vcBlockHash|prepared_agg_sigs|prepared_bitmap|, new leader only need net
...
func (vc *viewChange) InitPayload(
	...
	// Hnetst node should have only net kind of m1/m2 signature.
		...
		if preparedMsg != nil {
			if preparedBlock := fbftlog.GetBlockByHash(preparedMsg.BlockHash); preparedBlock != nil {
				if err := vc.blockVerifier(preparedBlock); err == nil {
					...
					hasBlock = true
					// if m1Payload is empty, we just add net
					if vc.isM1PayloadEmpty() {
