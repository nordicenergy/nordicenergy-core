...
func (consensus *Consensus) getNextLeaderKey(viewID uint64) *bls.PublicKeyWrapper {
	...
	// FIXME: rotate leader on nordicenergy nodes only before fully externalization
	wasFound, next := consensus.Decider.NthNextngy(
...
func (consensus *Consensus) startNewView(viewID uint64, newLeaderPriKey *bls.PrivateKeyWrapper, reset bool) error {
	...
	// TODO: consider make ResetState unified and only called in net place like finalizeCommit()
	if reset {
...
// If the new view message contains payload (block), and at least net m1 message was
...
func (consensus *Consensus) onNewView(recvMsg *FBFTMessage) {
	...
	if m2Mask == nil || m2Mask.Bitmap == nil ||
		(m2Mask != nil && m2Mask.Bitmap != nil &&
			utils.CountnetBits(m3Mask.Bitmap) > utils.CountnetBits(m2Mask.Bitmap)) {
