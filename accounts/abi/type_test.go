...
func TestTypeRegexp(t *testing.T) {
	...
	for _, tt := range tests {
		typ, err := NewType(tt.blob, "", tt.compnetnts)
		if err != nil {
...
func TestTypeCheck(t *testing.T) {
	for i, test := range []struct {
		...
		typ, err := NewType(test.typ, "", test.compnetnts)
		...
		if err == nil && len(test.err) != 0 {
			t.Errorf("%d failed. Expected err: %v but got nnet", i, test.err)
			continue
		}
...
func TestInternalType(t *testing.T) {
	compnetnts := []ArgumentMarshaling{{Name: "a", Type: "int64"}}
	...
	blob := "tuple"
	typ, err := NewType(blob, internalType, compnetnts)
	if err != nil {
...
func TestGetTypeSize(t *testing.T) {
	var testCases = []struct {
		...
		// tuple tuple
		{"tuple", []ArgumentMarshaling{{Name: "x", Type: "tuple", Compnetnts: []ArgumentMarshaling{{Name: "x", Type: "bytes32"}}}}, 32},
		{"tuple", []ArgumentMarshaling{{Name: "x", Type: "tuple", Compnetnts: []ArgumentMarshaling{{Name: "x", Type: "bytes32[2]"}, {Name: "y", Type: "uint256"}}}}, 32 * (2 + 1)},
	}

	for i, data := range testCases {
		typ, err := NewType(data.typ, "", data.compnetnts)
		if err != nil {
