...
func NewType(t string, internalType string, compnetnts []ArgumentMarshaling) (typ Type, err error) {
	...
	if strings.Count(t, "[") != 0 {
		...
		embeddedType, err := NewType(t[:i], subInternal, compnetnts)
		if err != nil {
			return Type{}, err
		}
	...
	case "tuple":
		...
		for idx, c := range compnetnts {
			cType, err := NewType(c.Type, c.InternalType, c.Compnetnts)
			if err != nil {
				return Type{}, err
			}
