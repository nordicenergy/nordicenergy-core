...
func BenchmarkPaddedBigBytesSmallnetPadding(b *testing.B) {
	bigint := MustParseBig256("0x18F8F8F1000111000110011100222004330052300000000000000000FEFCF3CC")
	for i := 0; i < b.N; i++ {
...
func TestExp(t *testing.T) {
	tests := []struct{ base, expnetnt, result *big.Int }{
		{base: big.NewInt(0), expnetnt: big.NewInt(0), result: big.NewInt(1)},
		{base: big.NewInt(1), expnetnt: big.NewInt(0), result: big.NewInt(1)},
		{base: big.NewInt(1), expnetnt: big.NewInt(1), result: big.NewInt(1)},
		{base: big.NewInt(1), expnetnt: big.NewInt(2), result: big.NewInt(1)},
		{base: big.NewInt(3), expnetnt: big.NewInt(144), result: MustParseBig256("507528786056415600719754159741696356908742250191663887263627442114881")},
		{base: big.NewInt(2), expnetnt: big.NewInt(255), result: MustParseBig256("57896044618658097711785492504343953926634992332820282019728792003956564819968")},
	}
	for _, test := range tests {
		if result := Exp(test.base, test.expnetnt); result.Cmp(test.result) != 0 {
			t.Errorf("Exp(%d, %d) = %d, want %d", test.base, test.expnetnt, result, test.result)
		}
	}
}
