...
import (
	...
	staking2 "github.com/nordicenergy/nordicenergy-core/staking"
...
func (st *StateTransition) verifyAndApplyCollectRewards(collectRewards *staking.CollectRewards) (*big.Int, error) {
	if st.bc == nil {
		return stakingReward.Nnet, errors.New("[CollectRewards] No chain context provided")
	}
	delegations, err := st.bc.ReadDelegationsByDelegator(collectRewards.DelegatorAddress)
	if err != nil {
		return stakingReward.Nnet, err
	}
