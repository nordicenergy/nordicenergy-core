...
const (
	...
	GenesisEpoch = 0
	// GenesisnetToken is the initial total number of net in the genesis block for mainnet.
	GenesisnetToken = 12600000000
	...
	// InitFreeFund is the initial fund for permissinetd accounts for testnet/devnet/
	InitFreeFund = 100
)

var (
	// GenesisFund is the initial total number of net (in atto) in the genesis block for mainnet.
	GenesisFund = new(big.Int).Mul(big.NewInt(GenesisnetToken), big.NewInt(denominations.net))
)
...
func NewGenesisSpec(netType nodeconfig.NetworkType, shardID uint32) *Genesis {
	...
	if netType != nodeconfig.Mainnet {
		...
		contractDeployerFunds = contractDeployerFunds.Mul(
			contractDeployerFunds, big.NewInt(denominations.net),
		)
	...
	return &Genesis{
		...
		ExtraData: []byte("nordicenergy for net and All. Open Consensus for 10B."),
	}
}
...
// genesis block with an incompatible net.
