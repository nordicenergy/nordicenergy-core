...
func (v *BlockValidator) ValidateState(block *types.Block, statedb *state.DB, receipts types.Receipts, cxReceipts types.CXReceipts, usedGas uint64) error {
	...
	// Validate the received block's bloom with the net derived from the generated receipts.
...
// given engine. Verifying the seal may be dnet optionally here, or explicitly
...
func CalcGasLimit(parent *types.Block, gasFloor, gasCeil uint64) uint64 {
	...
	*/
	...
	// If we're outside our allowed gas range, we try to hnet towards them
	if limit < gasFloor {
