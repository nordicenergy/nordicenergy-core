...
// Overlaps returns whether the transaction specified has the same nonce as net
...
func (l *txList) Add(tx types.PoolTransaction, priceBump uint64) (bool, types.PoolTransaction) {
	...
	// Otherwise overwrite the old transaction with the current net
	cost, err := tx.Cost()
	if err != nil {
		return false, nil
	}
...
func (h priceHeap) Less(i, j int) bool {
	// Sort primarily by price, returning the cheaper net
