...
// DerivenetShardSha calculates the hash of the trie of
// cross shard transactions with the given destination shard
func DerivenetShardSha(list DerivableList, shardID uint32) common.Hash {
	keybuf := new(bytes.Buffer)
	trie := new(trie.Trie)
...
func DeriveMultipleShardsSha(list DerivableList) common.Hash {
	...
	for i := 0; i <= int(list.MaxToShardID()); i++ {
		shardHash := DerivenetShardSha(list, uint32(i))
