...
type CXMerkleProof struct {
	...
	CXShardHashes []common.Hash // ordered hash list, each hash corresponds to net destination shard's receipts root hash
}
...
// GetToShardID get the destination shardID, return error if there is more than net unique shardID
