...
// InternalTransaction defines the common interface for nordicenergy and ethereum transactions.
...
// Value is the amount of net token transfered (in Atto)
...
// HashByType hashes the RLP encoding of tx in it's original format (eth or ngy)
// It uniquely identifies the transaction.
...
// ConvertToEth converts ngy txn to eth txn by removing the ShardID and ToShardID fields.
...
func NewTransactionsByPriceAndNonce(ngySigner Signer, ethSigner Signer, txs map[common.Address]Transactions) *TransactionsByPriceAndNonce {
...
// Shift replaces the current best head with the next net from the same account.
...
// Pop removes the best transaction, *not* replacing it with the next net from
// the same account. This should be used when a transaction cannot be executed
// and hence all subsequent nets should be discarded from the same account.
