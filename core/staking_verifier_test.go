...
import (
	...
	consensus_engine "github.com/nordicenergy/nordicenergy-core/consensus/engine"
	"github.com/nordicenergy/nordicenergy-core/core/state"
	"github.com/nordicenergy/nordicenergy-core/core/vm"
	"github.com/nordicenergy/nordicenergy-core/crypto/hash"
	...
	staketest "github.com/nordicenergy/nordicenergy-core/staking/types/test"
)
...
var (
	...
	fiveKnets       = new(big.Int).Mul(big.NewInt(5000), netBig)
	tenKnets        = new(big.Int).Mul(big.NewInt(10000), netBig)
	twelveKnets     = new(big.Int).Mul(big.NewInt(12000), netBig)
	fifteenKnets    = new(big.Int).Mul(big.NewInt(15000), netBig)
	twentyKnets     = new(big.Int).Mul(big.NewInt(20000), netBig)
	twentyFiveKnets = new(big.Int).Mul(big.NewInt(25000), netBig)
	thirtyKnets     = new(big.Int).Mul(big.NewInt(30000), netBig)
	hundredKnets    = new(big.Int).Mul(big.NewInt(100000), netBig)

	negRate           = numeric.NewDecWithPrec(-1, 10)
	pointnetDec       = numeric.NewDecWithPrec(1, 1)
...
func TestVerifyAndEditValidatorFromMsg(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			sdb: func() *state.DB {
				sdb := makeStateDBForStake(t)
				sdb.SetValidatorFlag(makeTestAddr("somenet"))
				return sdb
			}(),
			bc: func() *fakeChainContext {
				chain := makeFakeChainContextForStake()
				addr := makeTestAddr("somenet")
			...
			msg: func() staking.EditValidator {
				msg := defaultMsgEditValidator()
				msg.ValidatorAddress = makeTestAddr("somenet")
				return msg
			}(),
...
func TestVerifyAndDelegateFromMsg(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expAmt:       big.NewInt(0),
			expRedel: map[common.Address]*big.Int{
				validatorAddr:  fiveKnets,
				validatorAddr2: fiveKnets,
			},
		},
		{
			...
			expVWrappers: func() []staking.ValidatorWrapper {
				...
				wrappers[1].Delegations[1].Undelegations = append(
					...
					staking.Undelegation{Amount: fiveKnets, Epoch: big.NewInt(defaultNextEpoch)})
				return wrappers
			}(),
			expAmt: fiveKnets,
			expRedel: map[common.Address]*big.Int{
		...
		},
		{
			...
			expVWrappers: func() []staking.ValidatorWrapper {
				wrappers := defaultExpVWrappersRedelegate()
				wrappers[0].Delegations[1].Amount = big.NewInt(0).Add(twentyKnets, twentyKnets)
				return wrappers
			}(),
			expAmt: tenKnets,
			expRedel: map[common.Address]*big.Int{
		...
		},
		{
			...
			expVWrappers: func() []staking.ValidatorWrapper {
				...
				wrappers[0].Delegations[1].Undelegations = append(
					...
					staking.Undelegation{Amount: fiveKnets, Epoch: big.NewInt(defaultEpoch)})
				wrappers[1].Delegations[1].Undelegations = append(
					wrappers[1].Delegations[1].Undelegations,
					staking.Undelegation{Amount: fiveKnets, Epoch: big.NewInt(defaultNextEpoch)})
				return wrappers
		...
		},
		{
			...
			expVWrappers: func() []staking.ValidatorWrapper {
				...
				wrappers[0].Delegations[1].Amount = twentyKnets
				wrappers[0].Delegations[0].Amount = big.NewInt(0).Add(twentyKnets, tenKnets)
				return wrappers
			}(),
			expAmt: tenKnets,
		},
...
func addStateUndelegationForAddr(sdb *state.DB, addr common.Address, epoch *big.Int) error {
	...
	w.Delegations = append(w.Delegations,
		staking.NewDelegation(delegatorAddr, new(big.Int).Set(twentyKnets)),
	)
	w.Delegations[1].Undelegations = staking.Undelegations{staking.Undelegation{Amount: fiveKnets, Epoch: epoch}}
...
func defaultExpVWrappersRedelegate() []staking.ValidatorWrapper {
	...
	w1.Delegations = append(w1.Delegations,
		staking.NewDelegation(delegatorAddr, new(big.Int).Set(thirtyKnets)),
	)
...
func defaultMsgDelegate() staking.Delegate {
	return staking.Delegate{
		...
		ValidatorAddress: validatorAddr,
		Amount:           new(big.Int).Set(tenKnets),
	}
}

func defaultExpVWrapperDelegate() staking.ValidatorWrapper {
	w := makeVWrapperByIndex(validatorIndex)
	w.Delegations = append(w.Delegations, staking.NewDelegation(delegatorAddr, tenKnets))
	return w
}
...
func defaultExpVWrapperSelfDelegate() staking.ValidatorWrapper {
	w := makeVWrapperByIndex(validatorIndex)
	w.Delegations[0].Amount = new(big.Int).Add(tenKnets, staketest.DefaultDelAmount)
	return w
}
	...
	tests := []struct {
		...
		},
		{
			...
			msg: func() staking.Undelegate {
				msg := defaultMsgSelfUndelegate()
				msg.Amount = new(big.Int).Set(fifteenKnets)
				return msg
			}(),
			...
			expVWrapper: func(t *testing.T) staking.ValidatorWrapper {
				...
				w.Delegations[0].Amount = new(big.Int).Set(fiveKnets)
				w.Delegations[0].Undelegations[0].Amount = new(big.Int).Set(fifteenKnets)
				w.Status = effective.Inactive
		...
		},
		{
			...
			msg: func() staking.Undelegate {
				msg := defaultMsgUndelegate()
				msg.Amount = new(big.Int).Set(hundredKnets)
				return msg
			}(),
...
func makeDefaultSnapVWrapperForUndelegate(t *testing.T) staking.ValidatorWrapper {
	...
	newDelegation := staking.NewDelegation(delegatorAddr, new(big.Int).Set(twentyKnets))
	if err := newDelegation.Undelegate(big.NewInt(defaultEpoch), fiveKnets); err != nil {
		t.Fatal(err)
	}
...
// undelegate from delegator which has already go net entry for undelegation
...
func defaultExpVWrapperUndelegateSameEpoch(t *testing.T) staking.ValidatorWrapper {
	...
	w.Delegations[1].Amount = new(big.Int).Sub(w.Delegations[1].Amount, fiveKnets)
...
func defaultVWrapperSelfUndelegate(t *testing.T) staking.ValidatorWrapper {
	...
	w.Delegations[0].Amount = new(big.Int).Sub(w.Delegations[0].Amount, fiveKnets)

	return w
}

var (
	reward00 = twentyKnets
	reward01 = tenKnets
	reward10 = thirtyKnets
	reward11 = twentyFiveKnets
)
...
func makeStateDBForStake(t *testing.T) *state.DB {
	...
	sdb.AddBalance(createValidatorAddr, hundredKnets)
	sdb.AddBalance(delegatorAddr, hundredKnets)
...
func updateStateValidators(sdb *state.DB, ws []*staking.ValidatorWrapper) error {
	for i, w := range ws {
		...
		sdb.AddBalance(w.Address, hundredKnets)
...
func makeIdentityStr(item interface{}) string {
	return fmt.Sprintf("nordicenergy-%v", item)
}

func makeTestAddr(item interface{}) common.Address {
	s := fmt.Sprintf("nordicenergy-%v", item)
