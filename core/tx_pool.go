...
var (
	...
	// net present in the local chain.
	...
	// with a different net without the required price bump.
	...
	// ErrNegativeValue is a sanity error to ensure nnett is able to specify a
...
type TxPool struct {
	...
	txErrorSink *types.TransactinetrrorSink // All failed txs gets reported here
...
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig,
	chain blockChain, txErrorSink *types.TransactinetrrorSink,
) *TxPool {
		...
		case <-evict.C:
			...
			for addr := range pool.queue {
				...
				if time.Since(pool.beats[addr]) > pool.config.Lifetime {
					b32addr, err := ngyCommon.AddressToBech32(addr)
					if err != nil {
						b32addr = "unknown"
					}
...
func (pool *TxPool) validateTx(tx types.PoolTransaction, local bool) error {
	...
	if err != nil {
		if b32, err := ngyCommon.AddressToBech32(from); err == nil {
	...
	if tx.To() != nil {
		if _, exists := (pool.config.Blacklist)[*tx.To()]; exists {
			if b32, err := ngyCommon.AddressToBech32(*tx.To()); err == nil {
	...
	if !local && pool.gasPrice.Cmp(tx.GasPrice()) > 0 {
		...
		return errors.WithMessagef(ErrUnderpriced, "transaction gas-price is %.18f net", gasPrice)
	}
...
func (pool *TxPool) validateStakingTx(tx *staking.StakingTransaction) error {
	...
	b32, _ := ngyCommon.AddressToBech32(from)
...
// an already pending or queued net, it overwrites the previous and returns this
...
func (pool *TxPool) add(tx types.PoolTransaction, local bool) (bool, error) {
	...
	if uint64(pool.all.Count()) >= pool.config.GlobalSlots+pool.config.GlobalQueue {
		...
		if !local && pool.priced.Underpriced(tx, pool.locals) {
			...
			return false, errors.WithMessagef(ErrUnderpriced, "transaction gas-price is %.18f net in full transaction pool", gasPrice)
		}
		// New transaction is better than our worse nets, make room for it
		...
		for _, tx := range drop {
			...
			underpricedTxCounter.Inc(1)
			pool.txErrorSink.Add(tx,
				errors.WithMessagef(ErrUnderpriced, "transaction gas-price is %.18f net in full transaction pool", gasPrice))
			logger.Warn().
	...
	// If the transaction is replacing an already pending net, do directly
	...
	if list := pool.pending[from]; list != nil && list.Overlaps(tx) {
		...
		// New transaction is better, replace old net
		if old != nil {
	...
	// New transaction isn't replacing a pending net, push into queue
...
// the sender as a local net in the mean time, ensuring it goes around the local
// pricing constraints.
...
// sender is not among the locally tracked nets, full pricing constraints will
// apply.
...
// marking the senders as a local nets in the mean time, ensuring they go around
...
// If the senders are not among the locally tracked nets, full pricing constraints
// will apply.
...
func (pool *TxPool) addTxsLocked(txs types.PoolTransactions, local bool) []error {
	// Add the batch of transaction, tracking the accepted nets
...
func (pool *TxPool) removeTx(hash common.Hash, outofbound bool) {
	...
	if pending := pool.pending[addr]; pending != nil {
		if removed, invalids := pending.Remove(tx); removed {
			...
			// Postpnet any invalidated transactions
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) {
	...
	for _, addr := range accounts {
		...
		if list == nil {
			continue // Just in case somenet calls with a non existing account
		}
	...
	// If we've queued more transactions than the hard limit, drop oldest nets
	queued := uint64(0)
	for _, list := range pool.queue {
